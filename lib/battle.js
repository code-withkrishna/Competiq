...
function dimension(name, scoreA, scoreB, companyA, companyB, insight) {
  ...
  return {
    ...
    score_a: clampedA,
    score_b: clampedB,
    winner: winnerFor(clampedA, clampedB, companyA, companyB),
    insight,
  };
}
...
function average(scores) {
  ...
  return clampScore(validScores.reduce((sum, score) => sum + score, 0) / validScores.length);
}
...
export function normalizeBattleReport(battle, companyA, companyB) {
  ...
  normalized.dimensions = safeArray(normalized.dimensions).map((item) => ({
    ...
    score_a: item?.score_a === null ? null : clampScore(item?.score_a, 0),
    score_b: item?.score_b === null ? null : clampScore(item?.score_b, 0),
    winner: item?.winner || winnerFor(item?.score_a, item?.score_b, companyA, companyB),
